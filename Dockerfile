...
RUN yum -y install emacs-nox perl perl-Getopt-Long bind-utils openssl nmap-ncat perl-Data-Compare perl-DateTime \
		   ...
		   perl-File-Find-Object perl-Module-CPANTS-Analyse gcc make perl-App-cpanminus.noarch jq curl unzip \
		   && yum clean all && rm -rf /var/cache/yum 
RUN cpanm Net::Amazon::Signature::V4 List::Util && cpanm Paws Paws::Credential::File && cpanm Paws \
    && rm -rf /root/.cpanm
RUN cd /usr/local/src && curl -s -L "https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip" -o "awscliv2.zip" && \
    unzip awscliv2.zip && ./aws/install && rm -rf awscliv2.zip aws && cd /tmp
